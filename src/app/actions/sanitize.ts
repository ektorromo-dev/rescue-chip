...
export async function updateProfileSafe(profileId: string, data: Record<string, any>) {
    ...
    // Sincronizar invitaciones de contacto
    if (sanitizedData.emergency_contacts && Array.isArray(sanitizedData.emergency_contacts)) {
        const riderName = (sanitizedData.full_name as string) || existingProfile.full_name || '';
        await syncContactInvitations(profileId, riderName, sanitizedData.emergency_contacts as EmergencyContactItem[]);
    }
