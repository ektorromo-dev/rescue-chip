...
export interface ProcessReportedChipScanParams {
    folio: string;
    ipAddress?: string | null;
    userAgent?: string | null;
}

export interface ProcessReportedChipScanResult {
    success: boolean;
    error?: string;
}

export async function processReportedChipScanAlert(
    params: ProcessReportedChipScanParams
): Promise<ProcessReportedChipScanResult> {
    const { folio, ipAddress, userAgent } = params;

    if (!folio || typeof folio !== "string") {
        return { success: false, error: "Folio requerido." };
    }
    ...
    try {
        ...
        if (chipError || !chip) {
            ...
            return { success: false, error: "Chip no encontrado." };
        }
        ...
        if (chip.status !== "reportado") {
            ...
            return { success: false, error: "El chip no está reportado." };
        }

        // 2. Normalizar IP y User Agent
        const ip_address = (ipAddress && ipAddress !== "127.0.0.1") ? ipAddress : "Desconocida";
        const user_agent = (userAgent && userAgent.trim() !== "") ? userAgent : "Desconocido";
        ...
        console.error("[alert-reported-scan] Error inesperado en processReportedChipScanAlert:", err);
        return { success: false, error: err?.message || "Error interno procesando alerta." };
    }
}
