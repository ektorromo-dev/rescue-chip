...
import { fetchRoute, type NavRoute } from './MapaRescueChip'
...
async function fetchWeatherSimple(lat: number, lng: number) {
  try {
    const r = await fetch(
...
export default function MapaDashboard() {
  ...
  const enterFullscreen = useCallback(async () => {
    setFullscreen(true)
    try { await fsRef.current?.requestFullscreen() } catch {}
  }, [])
  ...
  // Sync ESC key and browser fullscreen exit
  useEffect(() => {
  ...
  // ── Navigation ───────────────────────────────────────────────────────────
  const calcRoute = async (invertir = false) => {
    setNavError(''); setNavLoading(true)
    try {
      let origenFinal = origenText.trim()
      let destinoFinal = destinoText.trim()
      if (invertir) [origenFinal, destinoFinal] = [destinoFinal, origenFinal]

      let origenCoords: { lng: number; lat: number; label: string } | null = null
      let destinoCoords: { lng: number; lat: number; label: string } | null = null

      // Origen: 'mi ubicación' o geocode
      if (!origenFinal || origenFinal.toLowerCase().includes('mi ubicación') || origenFinal.toLowerCase().includes('ubicacion')) {
        origenCoords = await new Promise((res) =>
          navigator.geolocation.getCurrentPosition(
        ...
        origenCoords = await geocode(origenFinal)
      }

      if (!destinoFinal) { setNavError('Ingresa un destino'); setNavLoading(false); return }
      destinoCoords = await geocode(destinoFinal)
      ...
      if (invertir) {
        setOrigenText(destinoFinal)
        setDestinoText(origenFinal)
      }
    ...
    } catch { setNavError('Error al calcular la ruta') }
    finally { setNavLoading(false) }
  }

  const invertirRuta = () => {
    if (!destinoText && !origenText) return
    calcRoute(true)
  }
  ...
  const navPanel = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ position: 'relative' }}>
        <input
          type="text"
          placeholder="🟢 Origen (vacío = mi ubicación)"
          value={origenText}
      ...
      <div style={{ display: 'flex', gap: '6px' }}>
        <button
          onClick={() => calcRoute()}
          disabled={navLoading}
          style={{ flex: 1, background: navLoading ? '#555' : '#E8231A', color: '#fff', border: 'none', padding: '10px', borderRadius: '6px', fontSize: '13px', fontWeight: 700, cursor: navLoading ? 'not-allowed' : 'pointer', letterSpacing: '0.5px' }}
        >
  ...
  if (fullscreen) {
    return (
      <div ref={fsRef} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#0A0A08' }}>
        <MapaRescueChip puntos={puntos} interactive height="100dvh" fullscreen navRoute={navRoute} />
        ...
        {/* FABs — bottom right, stacked, above safe area */}
        <div style={{
          ...
          {!reportOpen && !navOpen && (
            <>
              <button onClick={() => setNavOpen(true)} style={{
                ...
                borderRadius: '22px', padding: '11px 18px',
                fontSize: '13px', fontWeight: 600, cursor: 'pointer',
                backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', gap: '7px',
              }}>
                🗺️ Ir a…
              </button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 14px', background: 'rgba(244,240,235,0.06)', border: '1px solid rgba(244,240,235,0.12)', borderRadius: '5px', color: '#F4F0EB', fontSize: '12px', cursor: 'pointer' }}
        >
          ⛶ Pantalla completa
        </button>
      </div>

      {/* Nav panel en vista normal */}
      ...
      {loading ? (
        ...
        <div style={{ borderRadius: '8px', overflow: 'hidden' }}>
          <MapaRescueChip puntos={puntos} interactive height="500px" navRoute={navRoute} />
        </div>
      )}
